...
async def set_stream_link(client, message):
    try:
        ...
        if link:
            ...
            await message.reply("Usage: /stream https://t.me/rkmovieszip or http://t.me/rkmovieszip")
    except IndexError:
        await message.reply("Usage: /stream https://t.me/rkmovieszip or http://t.me/rkmovieszip")
        return
...
async def m_grp(client, message):
    links = []
    link = await client.ask(message.chat.id ,"send me your pm search grp link or send /skiplink to skip , default is rkmoviesmain")
    if link.text == "/skiplink":
        links.append("https://t.me/rkmoviesmain")
    else:
        if checkIfLinkIsValid(link.text):
            links.append(link.text)
        else:
            await message.reply("Invalid link")
    link1 = await client.ask(message.chat.id ,"send me your movies grp link or send /skiplink to skip . default is rkmoviesmain")
